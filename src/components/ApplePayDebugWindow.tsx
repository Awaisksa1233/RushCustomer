  ...
  const testValidateSessionApi = async () => {
    ...
    try {
      ...
      const res = await fetch("/api/applepay/validate-session", {
        ...
        body: JSON.stringify({
          validationUrl: "https://apple-pay-gateway.apple.com/acws/v2/merchantSession",
        }),
      });
      ...
      if (res.ok && !data.error) {
        onAddLog({
          type: "success",
          message: `API /api/applepay/validate-session responded in ${elapsed}ms`,
          details: { status: res.status, data },
        });
      } else if (data.appleStatusCode === 503 || data.raw?.includes("503 Service Temporarily Unavailable")) {
        onAddLog({
          type: "success",
          message: `✔ Certificate Handshake Successful! (Backend cert & key loaded cleanly). Apple returned 503 as expected for static test URL.`,
          details: {
            status: res.status,
            note: "Apple Pay validation URLs are single-use ephemeral URLs generated by ApplePaySession.onvalidatemerchant during an active Safari sheet payment session. Static test URLs are rejected by Apple with 503 HTML once TLS handshake finishes.",
            data,
          },
        });
      } else {
        onAddLog({
          type: "error",
          message: `API returned error status ${res.status}: ${data.error || data.message || "Unknown error"}`,
          details: { status: res.status, data },
        });
      }
    } catch (err: any) {
